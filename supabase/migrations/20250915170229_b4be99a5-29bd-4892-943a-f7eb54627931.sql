...
-- 5. Add RLS policies to views for proper access control
-- RLS for security_monitoring_dashboard view
CREATE POLICY "Only owner can view monitoring dashboard" 
ON public.security_monitoring_dashboard 
FOR SELECT 
USING (get_user_role(auth.uid()) = 'owner'::user_role);

-- RLS for service_provider_costs_masked view  
CREATE POLICY "Only owner can view masked costs" 
ON public.service_provider_costs_masked 
FOR SELECT 
USING (get_user_role(auth.uid()) = 'owner'::user_role);

-- 6. Fix public table policies - Restrict cash_flow_categories
...
-- 7. Restrict system_settings to proper roles only
...
-- 8. Add additional security layer for sensitive data access
...
-- 9. Create index for better audit log performance
...
-- 10. Add column to track failed access attempts
...
-- 11. Create function to track failed access attempts
...
-- 12. Reset failed attempts on successful access
